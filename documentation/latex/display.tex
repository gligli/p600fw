The 2 digit 7 segment display is used for different purposes depending on the mode and selected function as shown below. There are three distinct types of information: display of numeric values (parameters values), scrolling of (short) texts, indicators symbols for pick-me-up in Preset Panel mode, etc.
...
\scalebox{0.35}{
  ...
  \node[font=\fontsize{26}{12}\selectfont, align=right, outer sep=0.5mm, anchor = west, text width=7cm] at (-8cm,21.0cm) {Types of display information:};
    ...
    \node[font=\fontsize{26}{12}\selectfont, align=left, outer sep=0.5mm, anchor = west, text width=28cm] at (14cm,-5cm) {...};
    \node[font=\fontsize{26}{12}\selectfont, align=left, outer sep=0.5mm, anchor = west, text width=28cm] at (24cm,-5cm) {Status of tuning procedure from oscillator A (1..6) through oscillator B (1...6) to filter (1..6), see section \ref{tuning} on tuning};
...
Note that while numeric values are shown in the range -50 to 48 or 0 to 98, the internally used accuracy and dial sensitivity is much higher. As a result of this, noticable patch changes can be achieved by turning a dial slightly while the display continues to show the same value. The display is a rough indication of where the current values. However, it can be used accurately when centering bipolar dials.
